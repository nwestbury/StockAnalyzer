...
class MorningStarScaper():
    ...
    def parse_csv(self, csv_r, num_cols, special_titles, column_map, extra_cols=[], extra_vals=[]):
        subhead = ''
        next(csv_r) # skip header

        col_names = extra_cols
        vals = [list(extra_vals) for _ in range(num_cols)] # Don't do, [[]] * 6
        for cols in csv_r:
            row_cols = len(cols)
            if row_cols == 0:
                continue
    ...
    def get_key_stats(self, ticker, db_exchange="TSX"):
        ...
        col_names, vals = self.parse_csv(csv_r, 10, self.special_key_titles, self.column_key_map,
                                         extra_cols=["ticker", "exchange", "update_date"],
                                         extra_vals=[ticker, db_exchange, self.today])

        self.db.insert_into("ms_key_statistics", col_names, vals, unique_conflict=True)
        LOGGER.info("Done")
        return 0
    ...
    def get_financial(self, ticker, period_name, exchange="XTSE"):
        ...
        if on_morningstar:
            LOGGER.info("Getting {} financial data for {}... ".format(period_name, ticker), end='')
        else:
            LOGGER.info("Skipping", ticker)
            return 1

        num_cols = 6 if period_name == "quarter" else 5 # skip last column if not quarter view (removes TTM)
        col_names, vals = self.parse_csv(csv_r, num_cols, self.special_financials_titles, self.column_financials_map,
                                         extra_cols=["ticker", "exchange", "period", "update_date"],
                                         extra_vals=[ticker, db_exchange, period, self.today])

        self.db.insert_into("ms_financials", col_names, vals, unique_conflict=True)
        LOGGER.info("Done")
        return 0

    def fetch_all(self, db_exchange):
        listings = self.db.select("ticker", "listings", where="exchange = %s AND (onms=TRUE OR onms IS NULL)", vals=[db_exchange], unroll=True)
        for ticker in listings:
            ...
            self.db.update("listings", ["onms"], [on_morningstar], "exchange=%s AND ticker=%s",
                           [db_exchange, ticker])
...
if __name__ == "__main__":
    ...
    mss.fetch_all("TSX")
    # mss.get_financial("IMO", "quarter")
    # mss.get_financial("BCE", "quarter")
